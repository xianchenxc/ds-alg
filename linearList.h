...
public:
  ...
  virtual void setData(int i, T &x) const = 0;           // 修改第 i 个表项的值为 x
  virtual bool Insert(int i, T &x) const = 0;            // 在第 i 个表项后插入 x
  virtual bool Remove(int i, T &x) const = 0;            // 删除第i个表项，通过 x 返回
